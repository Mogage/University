...
fun LoginScreen(onClose: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(
            ...
            if (loginUiState.authenticationCompleted || loginUiState.isDownloading) {
                Text(text = "Downloading... ${loginUiState.downloaded}/${loginUiState.toDownload}")
            }
            if (loginUiState.authenticationError != null) {
                Text(text = "Login failed ${loginUiState.authenticationError.message}")
            }
            if (loginUiState.downloadError != null) {
                Text(text = "Download failed ${loginUiState.downloadError.message}")
                Button(onClick = { /*TODO*/ }) {
                    Text(text = "Retry")
                }
            }
    ...
    LaunchedEffect(loginUiState.downloadCompleted) {
        Log.d(TAG, "Auth completed = ${loginUiState.downloadCompleted}");
        if (loginUiState.downloadCompleted) {
             onClose();
        }
    }
}
