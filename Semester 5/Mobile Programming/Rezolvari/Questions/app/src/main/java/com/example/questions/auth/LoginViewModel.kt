...
class LoginViewModel(
    ...
    fun downloadQuestion(questionId: Int) {
        viewModelScope.launch {
            try {
                if (!uiState.downloadResumable) {
                    questionRepository.clearDao()
                ...
                Log.d(TAG, "downloadQuestion - downloading question $questionId at ${System.currentTimeMillis()}")
                questionRepository.get(questionId)

                uiState = uiState.copy(downloadedQuestions = lastDownloadedIndex + 1)
                lastDownloadedIndex++
                if (lastDownloadedIndex == uiState.totalQuestions) {
                    Log.d(TAG, " lastDownloadedIndex downloadQuestion - downloaded all questions at ${System.currentTimeMillis()}")
                    uiState = uiState.copy(downloading = false)
                }

                Log.d(TAG, "downloadQuestion - downloaded question $questionId at ${System.currentTimeMillis()}")
            } catch (e: Exception) {
                Log.e(TAG, "downloadQuestion - downloaded question $questionId FAILED at ${System.currentTimeMillis()}")
