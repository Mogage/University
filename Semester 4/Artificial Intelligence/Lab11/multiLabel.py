...
def run():
    train_dir = "multilabel_modified/images"
    train_csv_path = "multilabel_modified/multilabel_classification(6)-reduced_modified.csv"
    train_df = pd.read_csv(train_csv_path)
    train_df["Image_Name"] = train_df["Image_Name"].map(lambda x: os.path.join(train_dir, x))

    train_df = train_df.sample(frac=0.05, random_state=21)

    train_set, dev_set = train_test_split(train_df, test_size=0.05, random_state=21)
    val_set, test_set = train_test_split(dev_set, test_size=0.05, random_state=21)
