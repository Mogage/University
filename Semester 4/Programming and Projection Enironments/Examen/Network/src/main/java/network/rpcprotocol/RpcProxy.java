...
public class RpcProxy implements IService {
    ...
    @Override
    public synchronized Collection<Game> getScores() throws Exception {
    ...
    @Override
    public synchronized DTOFinishedGame getFinishedGameInfo(int id) {
    ...
    public String move(int id, int row, int column) {
        Request request = new Request.Builder().type(RequestType.MOVE).data(new int[]{id, row, column}).build();
        try {
            sendRequest(request);
            Response response = readResponse();
            switch (response.type()) {
